...
export const useKonamiCode = (callback: () => void) => {
  ...
  const performKeySequenceCheck = useCallback((newKeys: string[]) => {
    let shouldReset = false

    if (newKeys.length === KONAMI_CODE.length) {
      const matchesFull = KONAMI_CODE.every((key, index) => key === newKeys[index])
      if (matchesFull) {
        shouldReset = true
      }
    }
    ...
    if (
      ...
      const matchesDir = DIRECTIONAL_CODE.every((key, index) => key === lastDirKeys[index])
      if (matchesDir) {
        shouldReset = true
      }
    }
