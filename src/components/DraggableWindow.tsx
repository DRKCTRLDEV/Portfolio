...
const convertToPixels = (value: PositionValue | SizeValue, dimension: 'width' | 'height'): number => {
...
const convertSizeToPixels = (size: Size): { width: number; height: number } => ({
...
export const DraggableWindow = ({ 
  ...
  const [previousState, setPreviousState] = useState({ position: initialPosPixels, size: initialSizePixels })
  ...
  const handleMouseMove = (e: MouseEvent) => {
    if (isDragging && !isMaximized) {
      ...
      if (resizeDirection.includes('right')) {
        ...
        newWidth = Math.max(minSizePixels.width, Math.min(maxSizePixels.width, Math.min(maxAllowedWidth, resizeStart.width + deltaX)))
      }
      if (resizeDirection.includes('left')) {
        const maxAllowedWidth = resizeStart.width + dragStart.x
        const proposedWidth = Math.max(minSizePixels.width, Math.min(maxSizePixels.width, Math.min(maxAllowedWidth, resizeStart.width - deltaX)))
        newX = dragStart.x + (resizeStart.width - proposedWidth)
        newWidth = proposedWidth
      }
      if (resizeDirection.includes('bottom')) {
        const maxAllowedHeight = window.innerHeight - dragStart.y
        newHeight = Math.max(minSizePixels.height, Math.min(maxSizePixels.height, Math.min(maxAllowedHeight, resizeStart.height + deltaY)))
      }
      if (resizeDirection.includes('top')) {
        const maxAllowedHeight = resizeStart.height + dragStart.y
        const proposedHeight = Math.max(minSizePixels.height, Math.min(maxSizePixels.height, Math.min(maxAllowedHeight, resizeStart.height - deltaY)))
  ...
  }, [isDragging, isResizing, dragStart, resizeStart, resizeDirection, position, size, minSize, maxSize, resizable])
  ...
  useEffect(() => {
    const handleWindowResize = () => {
      ...
      setSize(prev => {
        const constrainedWidth = Math.max(newMinSizePixels.width, Math.min(newMaxSizePixels.width, Math.min(prev.width, window.innerWidth)))
        const constrainedHeight = Math.max(newMinSizePixels.height, Math.min(newMaxSizePixels.height, Math.min(prev.height, window.innerHeight)))
        return {
  ...
  }, [isOpen, size.width, size.height, isMinimized, isMaximized, minSize, maxSize])
  ...
  return (
    ...
    >
      ...
      <div className={`backdrop-blur-md border rounded-xl shadow-2xl overflow-visible h-full flex flex-col transition-colors duration-200 ${
        theme === 'dark' 
              ...
              {showControls && (
                <>
                  ...
                  <button
                    ...
                    title={isMaximized ? "Cannot minimize when maximized" : "Minimize"}
                  />
                  <button
